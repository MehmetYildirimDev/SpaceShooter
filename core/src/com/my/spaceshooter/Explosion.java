...
class Explosion {
    ...
    //convert to ID array
    TextureRegion[] textureRegionsID = new TextureRegion[16];

    int index = 0;
    for (int i=0; i<4;i++){
        for (int j=0; j<4; j++) {
            textureRegionsID[index] = textureRegions2D[i][j];
            index++;
        }
    }

    explosionAnimation = new Animation<TextureRegion>(totalAnimationTime/16,textureRegionsID);
    explosionTimer =0;
    }
